...
function createCompleteFlattenedContract() {
...
contract PollFactory is Ownable {
  ...
  // Save the complete flattened contract
  const verifyDir = path.join(__dirname, '../verification-info');
  if (!fs.existsSync(verifyDir)) {
    fs.mkdirSync(verifyDir);
  }
  
  fs.writeFileSync(path.join(verifyDir, 'Complete-Flattened.sol'), content);
  console.log('✅ Created Complete-Flattened.sol with both PollFactory and Poll contracts');
  
  // Also create a version with just PollFactory that references Poll
  const factoryOnly = content.split('// File: contracts/PollFactory.sol')[1];
  fs.writeFileSync(path.join(verifyDir, 'PollFactory-WithDependencies.sol'), 
    '// SPDX-License-Identifier: MIT\npragma solidity ^0.8.24;\n\n' + 
    content.substring(0, content.indexOf('// File: contracts/PollFactory.sol')) +
    '// File: contracts/PollFactory.sol' + factoryOnly
  );
  
  console.log('✅ Created PollFactory-WithDependencies.sol');
  
  console.log('\n📋 Instructions:');
  console.log('1. Use Complete-Flattened.sol for verification on BaseScan');
  console.log('2. This includes both PollFactory and Poll contracts');
  console.log('3. BaseScan will recognize the factory-child relationship');
  console.log('4. All future Polls deployed will be auto-verified!');
}

createCompleteFlattenedContract();